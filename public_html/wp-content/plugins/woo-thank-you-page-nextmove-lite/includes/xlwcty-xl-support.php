...
class XLWCTY_XL_Support {
	...
	public function __construct() {
		...
		//add_action( 'admin_init', array( $this, 'xlwcty_xl_parse_request_and_process' ), 15 );
...
//		add_action( 'xl_support_system_info', array( $this, "xl_support_system_info" ) );
	...
	public function xlwcty_xl_expected_slug() {
		...
		if ( ! XL_admin_notifications::has_notification( 'license_needs_attention' ) && count( $invalid_licenses ) > 0 ) {
			$license_invalid_text = sprintf( __( '<p>You are <strong>not receiving</strong> Latest Updates, New Features, Security Updates &amp; Bug Fixes for <strong>%s</strong>. <a href="%s">Click Here To Fix This</a>.</p>', 'woo-thank-you-page-nextmove-lite' ), implode( ",", $invalid_licenses ), add_query_arg( array( 'tab' => 'licenses' ), $this->expected_url ) );
	...
	public function xlwcty_xl_parse_request_and_process() {
			...
			if ( filter_input( INPUT_POST, 'choose_addon' ) == "" || filter_input( INPUT_POST, 'comments' ) == "" ) {
				...
				XL_admin_notifications::add_notification( array(
					'support_request_failure' => array(
						'type'           => 'error',
						'is_dismissable' => true,
						'content'        => __( '<p> Unable to submit your request.All fields are required. Please ensure that all the fields are filled out.</p>', 'woo-thank-you-page-nextmove-lite' ),
					)
				) );
			} else {
	...
	public function xlwcty_options_page_right_content() {
		...
		if ( is_array( $other_products ) && count( $other_products ) > 0 ) {
			...
			if ( true === $easter_offer ) {
                ...
                <div class="postbox xlwcty_side_content xlwcty_xlplugins xlwcty_xlplugins_easter_offer">
                    <a href="<?php echo $offer_link ?>" target="_blank"></a>
                    <img src="<?php echo plugin_dir_url( XLWCTY_PLUGIN_FILE ) . 'admin/assets/img/easter.png'; ?>"/>
                    <div class="xlwcty_plugin_head">EASTER SPL.: UPTO 20% OFF!</div>
            ...
            <form method="post">
                ...
                <div class="xl_core_tools_button" style="margin-bottom: 10px;">
                    <a class="button button-primary button-large xl_core_tools_btn" data-plugin="nextmove-lite" href="<?php echo add_query_arg( array(
						...
						"download" => "true"
					), admin_url( "export.php" ) ) ?>"><?php echo __( "Export Thank You Pages", 'woo-thank-you-page-nextmove-lite' ) ?></a>
