...
class xlwcty {
	...
	public function init() {
		...
		add_action( 'wp', array( XLWCTY_Core()->data, 'load_order' ), 10 );
		...
		add_filter( 'template_include', array( $this, 'redirect_to_choosed_template' ) );
		...
		add_action( 'parse_query', array( $this, 'parse_query_for_thankyou' ), 1 );
	...
	public function redirect_to_choosed_template( $template ) {
		...
		if ( ! is_admin() ) {
			if ( $post instanceof WP_Post && $post->post_type == xlwcty_Common::get_thank_you_page_post_type_slug() ) {
				...
				if ( $tem == '' ) {
					$tem = get_post_meta( $post->ID, '_xlwcty_frontend_template', true );
				}
				if ( $tem != '' ) {
					$out = get_query_template( 'page', $tem );
	...
	public function validate_request() {
		if ( is_singular( XLWCTY_Common::get_thank_you_page_post_type_slug() ) && $this->is_preview === false && ( filter_input( INPUT_GET, 'order_id' ) == null || filter_input( INPUT_GET, 'key' ) == null ) ) {
		...
		if ( is_singular( XLWCTY_Common::get_thank_you_page_post_type_slug() ) && current_user_can( 'manage_woocommerce' ) ) {
			if ( filter_input( INPUT_GET, 'order_id' ) === null ) {
				...
				if ( $get_chosen_order_meta === '' ) {
					...
					$link             = add_query_arg( array(
						'order_id' => XLWCTY_Compatibility::get_order_id( $get_chosen_order ),
	...
	public function validate_order() {
		...
		if ( $order instanceof WC_Order ) {
			...
			if ( ! in_array( $current_order_status, XLWCTY_Core()->data->get_option( 'allowed_order_statuses' ) ) ) {
				/**
	...
	public function execute_wc_thankyou_hooks() {
		...
		?>
        <div class="xlwcty_wc_thankyou" style="display: none; opacity: 0"><?php
