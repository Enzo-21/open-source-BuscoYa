...
//        add_filter('cmb2_init', array($this, 'xlwcty_add_options_quick_view_metabox'));
...
//        add_action('do_meta_boxes', array($this, 'xlwcty_do_meta_boxes'), 999, 3);
...
//        add_action("admin_menu", array($this, "xlwcty_add_menu_page"));
	...
	public static function add_metaboxes() {
		add_meta_box( 'xlwcty_rules', 'Rules', array( __CLASS__, 'rules_metabox' ), XLWCTY_Common::get_thank_you_page_post_type_slug(), 'normal', 'high' );
	}
	...
	public function xlwcty_add_options_quick_view_metabox() {
		XLWCTY_Admin_Post_Options::quick_view_metabox_fields();
	}
	...
	public function xlwcty_enqueue_admin_assets() {
		if ( XLWCTY_Common::is_load_admin_assets( 'all' ) ) {
			...
			wp_localize_script( 'xlwcty_admin-js', 'xlwcty_nonces', array( 'xlwcty_get_pages_for_order' => wp_create_nonce( 'xlwcty_get_pages_for_order' ) ) );
			...
			if ( wp_verify_nonce( $_GET['_wpnonce'], 'xlwcty-post-activate' ) ) {
				...
				if ( $postID ) {
					...
					if ( isset( $_GET['paged'] ) && ! empty( $_GET['paged'] ) ) {
						$redirect_url = add_query_arg( array( "paged" => $_GET['paged'] ), $redirect_url );
					}
		...
		if ( isset( $_GET['action'] ) && $_GET['action'] == "xlwcty-post-deactivate" ) {
			...
			if ( wp_verify_nonce( $_GET['_wpnonce'], 'xlwcty-post-deactivate' ) ) {
				...
				if ( $postID ) {
					...
					wp_update_post( array( 'ID' => $postID, 'post_status' => XLWCTY_SHORT_SLUG . 'disabled' ) );
					$redirect_url = admin_url( 'admin.php?page=wc-settings&tab=' . XLWCTY_Common::get_wc_settings_tab_slug() . '&section=' . $section );
					if ( isset( $_GET['paged'] ) && ! empty( $_GET['paged'] ) ) {
						$redirect_url = add_query_arg( array( "paged" => $_GET['paged'] ), $redirect_url );
					}
		...
		if ( $post != null ) {
			if ( $post && $post->post_type == XLWCTY_Common::get_thank_you_page_post_type_slug() ) {
				if ( isset( $_POST['_xlwcty_menu_order'] ) ) {
					...
					if ( ! has_shortcode( $post->post_content, 'xlwcty_load' ) ) {
						$content = ( isset( $_POST['content'] ) ? $_POST['content'] : '' );
						wp_update_post( array( 'ID' => $post_id, 'menu_order' => $_POST['_xlwcty_menu_order'], 'post_content' => '[xlwcty_load]' . $content ) );
					} else {
						wp_update_post( array( 'ID' => $post_id, 'menu_order' => $_POST['_xlwcty_menu_order'] ) );
					}
				}
	 ...
	 * @param type $post
	 */
	public function xlwcty_do_meta_boxes( $post_type, $cur_context, $post ) {
		global $wp_meta_boxes;
		if ( $post_type == 'xlwcty_thankyou' ) {
			$allowed_side_metaboxes   = array( 'xlwcty_shortcode_settings', 'xlwcty_menu_order_settings' );
			$allowed_normal_metaboxes = array( 'xlwcty_settings', 'xlwcty_rules' );

			if ( isset( $wp_meta_boxes['xlwcty_thankyou']['side']['high'] ) ) {
				unset( $wp_meta_boxes['xlwcty_thankyou']['side']['high'] );
			}
			if ( isset( $wp_meta_boxes['xlwcty_thankyou']['advanced'] ) ) {
				unset( $wp_meta_boxes['xlwcty_thankyou']['advanced'] );
			}
			if ( isset( $wp_meta_boxes['xlwcty_thankyou']['normal']['low'] ) ) {
				unset( $wp_meta_boxes['xlwcty_thankyou']['normal']['low'] );
			}
			if ( is_array( $wp_meta_boxes['xlwcty_thankyou']['side']['low'] ) && count( $wp_meta_boxes['xlwcty_thankyou']['side']['low'] ) > 0 ) {
				$meta_box_keys = array_keys( $wp_meta_boxes['xlwcty_thankyou']['side']['low'] );
				if ( is_array( $meta_box_keys ) && count( $meta_box_keys ) > 0 ) {
					foreach ( $meta_box_keys as $metabox_id ) {
						if ( ! in_array( $metabox_id, $allowed_side_metaboxes ) ) {
							unset( $wp_meta_boxes['xlwcty_thankyou']['side']['low'][ $metabox_id ] );
						}
					}
				}
			}
			$meta_box_keys = array();
			if ( is_array( $wp_meta_boxes['xlwcty_thankyou']['normal']['high'] ) && count( $wp_meta_boxes['xlwcty_thankyou']['normal']['high'] ) > 0 ) {
				$meta_box_keys = array_keys( $wp_meta_boxes['xlwcty_thankyou']['normal']['high'] );
				if ( is_array( $meta_box_keys ) && count( $meta_box_keys ) > 0 ) {
					foreach ( $meta_box_keys as $metabox_id ) {
						if ( ! in_array( $metabox_id, $allowed_normal_metaboxes ) ) {
							unset( $wp_meta_boxes['xlwcty_thankyou']['normal']['high'][ $metabox_id ] );
						}
					}
				}
			}
	...
	public function xlwcty_wc_admin_menu() {
		add_submenu_page( 'woocommerce', __( 'Thank You Page', 'woo-thank-you-page-nextmove-lite' ), __( 'Thank You Page', 'woo-thank-you-page-nextmove-lite' ), 'manage_woocommerce', 'admin.php?page=wc-settings&tab=' . XLWCTY_Common::get_wc_settings_tab_slug() . '', false, false );
	}
	...
	public function xlwcty_add_menu_page() {
		add_menu_page( "Woocommerce Thankyou pages", "Woocommerce Thankyou pages", "manage_options", "xlwcty-single", array( $this, "xlwcty_admin_page" ) );
	}

	public function xlwcty_admin_page() {
		include_once __DIR__ . "/views/xlwcty_admin.php";
	}
	...
	public function get_last_order( $id, $is_data = false ) {
		...
		if ( ! empty( $get_chosen_order ) && $get_chosen_order > 0 ) {
			...
			if ( $selected_order instanceof WC_Order ) {
				...
				$data[]       = array( 'key' => $order_key, 'label' => $label, 'value' => XLWCTY_Compatibility::get_order_id( $selected_order ) );
			}
		}
		...
		if ( is_array( $orders ) && count( $orders ) > 0 ) {
			foreach ( $orders as $order ) {
				...
				$data[] = array( 'key' => $order_key, 'label' => $label, 'value' => XLWCTY_Compatibility::get_order_id( $order ) );
				if ( $is_data ) {
					$pre_data[] = array( 'text' => $label, 'value' => XLWCTY_Compatibility::get_order_id( $order ) );
				}
			}
		}
		if ( $is_data ) {
