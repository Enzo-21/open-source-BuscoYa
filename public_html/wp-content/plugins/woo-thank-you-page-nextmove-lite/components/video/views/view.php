						...
						if ( strpos( $this->data->url, 'youtu' ) !== false ) {
							...
							if ( ! empty( $youtube_id ) ) {
								...
								if ( strpos( $this->data->url, 'autoplay' ) !== false ) {
									$youtube_url = add_query_arg( array( 'autoplay' => '1' ), $youtube_url );
								}
								if ( strpos( $this->data->url, 'showinfo' ) !== false ) {
									$youtube_url = add_query_arg( array( 'showinfo' => '0' ), $youtube_url );
								}
								if ( strpos( $this->data->url, 'rel' ) !== false ) {
									$youtube_url = add_query_arg( array( 'rel' => '0' ), $youtube_url );
								}
								if ( strpos( $this->data->url, 'controls' ) !== false ) {
									$youtube_url = add_query_arg( array( 'controls' => '0' ), $youtube_url );
								}
								echo sprintf( '<iframe src="%s" width="1020" height="574" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>', $youtube_url );
							}
							...
							if ( ! empty( $vimeo_id ) ) {
								...
								if ( strpos( $this->data->url, 'autoplay' ) !== false ) {
									$vimeo_url = add_query_arg( array( 'autoplay' => '1' ), $vimeo_url );
								}
								if ( strpos( $this->data->url, 'loop' ) !== false ) {
									$vimeo_url = add_query_arg( array( 'loop' => '1' ), $vimeo_url );
								}
